...
The State class must have a constructor with the following signature:
    public State(String lbl)
All State instances default to reject (not accept).

The Transition class must have a constructor with the following signature:
    public Transition(T sym, StateI<T> n)

To practice working with file formats a bit more, you will also be writing the DFAGen class, which must implement StateMachineGenI. DFAGen reads files and streams conforming to the format described in README_dfa.txt. For the initial assignment, only files/streams with syntactically correct contents will be used for the first round of testing. In the tests for the next assignment expect your code to detect a whole bunch of specific errors in the data files and throw an InvalidStateMachineException with an informative message for each. It is strongly recommended you add comments for or add the throw statements as you consider what could go wrong while initially writing the code.

#################
# Included executables
#################
DFAMatcher - This program uses your DFAGen to generate a DFA to match strings against
WordSearcher - This program my DictionaryGen to generate a DFA to match strings against

#################
# For the next assignment...
#################
Exception conditions will not be checked for until the second assignment, but you might want to think about how you'll detect these problems as you work on this assignment. Adding error checking/securty after development is almost always much harder than including it during the first write of the code. A short description of the error conditions with expected message text follows.
...
There is more than one state declared to be the start state
    "Duplicate start state declared"
...
A line representing a transition involving a state not defined on an earlier line
    ...
    "Destination ("+destinationLabel+") for the transition is unknown"
...
src/statemachine/TraverserI.java        - Interface for statemachine executors
src/statemachine/TransitionI.java       - Interface for Transition implementations
src/statemachine/StateI.java            - Interface for State implementations
src/statemachine/DFATraverser.java      - Executes a DFA
src/statemachine/IterableString.java    - Wraps a String for iteration
src/fileaccess/StateMachineGenI.java  - Interface for things that generate state machines from files
src/fileaccess/DFAGen.java            - Starter for code to read .dfa files
src/fileaccess/DictionarGen.java      - Makes DFA from  file lines
src/fileaccess/InvalidStateMachineException.java - Signals there's a problem with the machine described in the file
src/programs/WordSearcher.java      - Program using DictionaryGen
src/programs/DFAMatcher.java        - Program using DFAGen
